<x-app-layout>

<!-- Animacion Header [Olas] -->
<section id="intro">
  <div class="header">

    <div class="inner-header flex">
      <!-- Eslogan -->
      <div class="headline">
        <h1>SOLUCIONES TECNOLÓGICAS <br> A LA MEDIDA</h1> 
      </div>
    </div>
<!--Olas Container-->
<div>
  <svg class="waves" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
    viewBox="0 24 150 28" preserveAspectRatio="none" shape-rendering="auto">
      <defs>
        <path id="gentle-wave" d="M-160 44c30 0 58-18 88-18s 58 18 88 18 58-18 88-18 58 18 88 18 v44h-352z" />
      </defs>

        <g class="parallax">
          <use xlink:href="#gentle-wave" x="48" y="0" fill="rgba(255,255,255,0.7" />
          <use xlink:href="#gentle-wave" x="48" y="3" fill="rgba(255,255,255,0.5)" />
          <use xlink:href="#gentle-wave" x="48" y="5" fill="rgba(255,255,255,0.3)" />
          <use xlink:href="#gentle-wave" x="48" y="7" fill="#fff" />
        </g>
  </svg>
</div>
<!--Olas Final-->
</div>
<!--Intro Final-->
</section>

<!-- Titulo Servicios -->
<section id="servicios">
<div class="titulo">
  <div class="glitch" data-text="Servicios">Servicios</div>
</div>

<!--Cartas-->
<div class="hero-section">
  <div class="card-grid">

    <!-- Carta 1 -->
    <div class="card">
        <div class="card__background" style="background-image: url(/img/bogservice1.jpg)"></div>
          <div class="card__content">
            <p class="card__category">Servicio</p>
            <h3 class="card__heading">Creación de Pagina Web</h3>
          </div>
        </div>

      <!-- Carta 2 -->
      <div class="card">
          <div class="card__background" style="background-image: url(/img/bogservice2.jpg)"></div>
            <div class="card__content">
              <p class="card__category">Servicio</p>
              <h3 class="card__heading">Asesoramiento sobre el mejor software para tu Empresa</h3>
            </div>
          </div>

      <div class="card">
          <div class="card__background" style="background-image: url(/img/bogservice3.jpg)"></div>
            <div class="card__content">
              <p class="card__category">Servicio</p>
              <h3 class="card__heading">Community Manager</h3>
            </div>
          </div>

  <div>
</div>
<!-- Final sección Cartas -->
</section>

<!-- Section Clientes-->
<section id="clientes">
  <!-- Div Imagen fondo Responsive -->
  <div class="bg-img2" style="background-image: url(/img/bogsclientes.jpg)">
    <!-- Titulo Clientes-->

   <!-- Inicio Carrusel Clientes -->
   <div class="masthead">
    <div class="masthead-content text-white">
      <div class="container-fluid px-4 px-lg-0">
        <!-- Titulo Seccion -->
        <h1 class="fst-italic lh-1 mb-4">Nuestros CLIENTES</h1>


          <div class="client-slider">
            <div class="client-slide-track">

            <!-- Carrusel Clientes [Logos] * 15  -->

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>

              <!-- Como en Kasa -->
              <div class="client-slide">
                <img src="/img/BogsCEK.png" height="100" alt="" />
              </div>

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>

              <!-- Como en Kasa -->
              <div class="client-slide">
                <img src="/img/BogsCEK.png" height="100" alt="" />
              </div>

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>
              <!-- Como en Kasa -->
              <div class="client-slide">
                <img src="/img/BogsCEK.png" height="100" alt="" />
              </div>

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>

              <!-- Como en Kasa -->
              <div class="client-slide">
                <img src="/img/BogsCEK.png" height="100" alt="" />
              </div>

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>

              <!-- Como en Kasa -->
              <div class="client-slide">
                <img src="/img/BogsCEK.png" height="100" alt="" />
              </div>

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>

              <!-- Como en Kasa -->
              <div class="client-slide">
                <img src="/img/BogsCEK.png" height="100" alt="" />
              </div>

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>

              <!-- Como en Kasa -->
              <div class="client-slide">
                <img src="/img/BogsCEK.png" height="100" alt="" />
              </div>

              <!-- Mega Estopas -->
              <div class="client-slide">
                <img src="/img/BogsME.png" height="100" alt="" />
              </div>


            </div>
          </div>

<!-- Final Carrusel -->
      </div>
    </div>
  </div>
</div>

</section>
<!--Content ends-->

<!-- Scroll Reveal JS -->
<script src="js/scroll.js"></script>

</x-app-layout>